"""
Tests for the query_code_call_graph MCP tool in berkelium_cli.mcp_server.
...
from berkelium_cli.mcp_server import query_code_call_graph
...
class TestQueryCodeCallGraph:
    ...
    def test_empty_cypher_returns_error(self, tmp_path):
        result = query_code_call_graph("", str(tmp_path))
    ...
    def test_nonexistent_repo_root_returns_error(self, tmp_path):
        result = query_code_call_graph(
            "MATCH (n) RETURN n", str(tmp_path / "no_such")
        )
        assert "error" in result
    ...
    def test_file_as_repo_root_returns_error(self, tmp_path):
        f = tmp_path / "file.py"
        f.write_text("x = 1")
        result = query_code_call_graph("MATCH (n) RETURN n", str(f))
        assert "error" in result
        assert "not a directory" in result["error"]

    def test_empty_graph_returns_error_with_hint(self, tmp_path):
        _make_store(tmp_path).close()
        result = query_code_call_graph("MATCH (n) RETURN n.name", str(tmp_path))
        assert "error" in result
